    ...
    <?php
        ...
        if(isset($_SESSION['username']) && $_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['old']) && isset($_POST['new'])){
            ...
            if($old_player!=null && ($user->getBalance() + $old_player->getValue() - $new_player->getValue())>=0){
                $error_code=$database->changePlayer($old_player,$new_player,$user,$players,$id_market);
            }
        ...
        if(!isset($_SESSION['username'])) {
            ...
            header("Location:login.php");
                ...
                $markets=$database->getOpenMarkets();
                ...
                if(isset($_GET['market']) || count($markets)==1 || isset($market_pre)){
                    ...
                    if(isset($_GET['market'])){
                        ...
                        $market=$database->getMarketById($id_market);
                    ...
                    $trans=$database->getTransfersByIdMarket($user,$players,$id_market);
            ...
            <div class="container-fluid">
	            <div class="row">
		            <div class="col-md-12">
			            ...
			            <?php
			                }else if(count($markets)==0){
			            ?>
			
			            <div class="error_display">Attenzione , nessuna sessione di mercato aperta</div>
			
			            <?php
			                }else{
			            ?>
			            <div id="team-info">
			                <form id="select-market-form" action="changeroster.php" method="get">
			                    <div class="name-team">Sono in corso <?php echo count($markets); ?> Mercati</div>
			                    <div class="balance">
			                        <select name="market" class="market-select">
			                        <?php foreach($markets as $mar){
			                            echo "<option value=\"".$mar->getId()."\" >".$mar->getName()."</option>";
			                        } ?>
			                        </select>
			                        <input class="market-select-button" type="submit" value="Seleziona Mercato">
			                    </div>
			                </form>
			            </div>
			            <?php
