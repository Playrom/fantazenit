...
function updateStats($stats_coll,$round){
		try{
			...
			foreach($stats_coll as $stats){
	            ...
	            $new_res=$mysqli->query($tempQuery);
...
function insertTeam($id_user,$ids,$reserves,$round,$tactic){
		try{
			...
			$players=dumpSingoliToList(null,null);
			...
			foreach($ids as $id){
				...
				if(!($stmt = $mysqli->prepare($prepQuery))) {
				    echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
				}
			...
			if(!($stmt = $mysqli->prepare($tacticQuery))) {
			    echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
			}
...
function calcRound($round){
		...
		try{
			...
			while ($row = $res->fetch_assoc()) {
				...
				$final_vote=calc($stats_coll,$row['role']);
	...
	function substitute($roling,$round,$alread_in,$max_sub,$step=0){
		$stat=dumpStatsByRound($roling[$step]->getPlayer()->getId(),$round);
		$nextSub=false;
		...
		return substitute($roling,$round,$alread_in,$max_sub,$step);
	}
	...
	function calcRoundUser($round){
			...
			try{
				...
				while ($row = $res->fetch_assoc()) {
					...
					$result=0;
					$id_user=$row['id'];
					$team=getTeam($id_user,$round);

					if($team->getPlayers()==null && $round>1 && getTeam($id_user,$round-1)->getPlayers()!=null){ // SE NON CE SQUADRA QUESTA GIORNATA MA PRECEDENTE SI
						$team=getTeam($id_user,$round-1);
			            ...
			            insertTeam($id_user,$ids,$reserves,$round,$tactic);
					...
					if($team->getPlayers()!=null){
						...
						foreach($start as $pla){
							...
							$stat=dumpStatsByRound($id_player,$round);
							...
							if($sub<$max_sub && $enterSub==true){
								...
								$arr=substitute($roling,$round,$alread_in,$max_sub);
								if($arr==0){
								}else{
									$subvote=$arr['vote'];
	...
	function unCalcRound($round){
		if(isCalcRound($round)){
			...
			try{
				...
				if(!($stmt = $mysqli->prepare($tempRemove))) {
				    echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
				}
